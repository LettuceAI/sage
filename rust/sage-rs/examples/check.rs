...
fn main() -> anyhow::Result<()> {
    ...
    let onnx = args.get(1).cloned().unwrap_or_else(|| "sage-v1-int8.onnx".into());
    let tok = args.get(2).cloned().unwrap_or_else(|| "tokenizer.json".into());
    ...
    let r = sage.check(&[
        Turn { role: Role::User, text: "Hey, what's your name?".into() },
        Turn { role: Role::Char, text: "I'm Alex!".into() },
        Turn { role: Role::User, text: "Are your parents home?".into() },
    ])?;
